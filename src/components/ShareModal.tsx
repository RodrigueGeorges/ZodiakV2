...
export function ShareModal({ isOpen, onClose, guidance, userName, url }: ShareModalProps) {
  ...
  const handleShareViaFacebook = () => {
    ...
    window.open(
      `https://www.facebook.com/sharer/sharer.php?u=${u}&quote=${encodeURIComponent(shareText)}`,
      '_blank'
    );
    onClose();
  };
  ...
  return (
    <motion.div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Partager ma guidance"
    >
      <motion.div
        ...
        <div className="space-y-3">
          {typeof navigator !== 'undefined' && 'share' in navigator && (
            <button
              ...
              className="w-full py-3 px-4 bg-primary text-cosmic-900 rounded-lg hover:opacity-90 transition-colors flex items-center gap-3 justify-center font-semibold"
            >
              Partager (système)
            </button>
          )}
          ...
          <button
            onClick={copyLink}
            className="w-full py-3 px-4 bg-white/5 hover:bg-white/10 transition-colors flex items-center gap-3 justify-center font-semibold text-primary rounded-lg"
          >
            {copied ? (
